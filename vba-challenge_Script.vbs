...
For i = 1 To numSheets
    'go to first/next sheet
    Sheets(i).Activate
    
    'how many records per sheet?
    lastRow = Cells(Rows.Count, 1).End(xlUp).Row
    
    'convert column B to date in new column C
    ...
    'label and format summary table columns
    'Column J = Ticker name
    With Range("J1")
        .Value = "Ticker"
        .ColumnWidth = 6
    End With
    
    'Column K = Yearly Change and requires conditional formatting:  green = greater than 0, red = less than 0.
    With Range("K1")
        .Value = "Yearly Change"
        .ColumnWidth = 13
    End With
    ...
    'conditional formating code acquired and modified from recording a macro.
